...
boolean ATECCX08A::signTempKeyDebug(uint16_t slot)
{
  ...
  delay(160); // time for IC to process command and exectute
...
boolean ATECCX08A::createSignatureDebug(uint8_t *data, uint16_t slot)
{
  boolean loadTempKeyResult = loadTempKeyDebug(data);
   _debugSerial->println("the loadtempKeyResult is");
  _debugSerial->println(loadTempKeyResult);
  _debugSerial->println(" ");
  boolean signTempKeyResult = signTempKeyDebug(slot);
  _debugSerial->println("the signTempKeyResult slot is");
  _debugSerial->println(signTempKeyResult);
  _debugSerial->println(" ");
  if(loadTempKeyResult && signTempKeyResult) return true;
  else return false;
}
...
boolean ATECCX08A::getStateInfo()
{
  sendCommand(COMMAND_OPCODE_INFO, 0x02, 0x0000); // param1 - 0x00 (revision mode).
  ...
  if(checkCount() == false) return false;
  if(checkCrc() == false) return false;
  if(inputBuffer[0] != 0x00) return true;   // If we hear a "0x50", that means it had a successful version response.
  else return false;
}
